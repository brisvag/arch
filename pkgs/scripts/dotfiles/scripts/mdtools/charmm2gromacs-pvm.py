...
# special type for treating multiple entries (modification in source code)
funcForDihedrals = '9'
funcForImpropers = '2'
...
for line in topFile:
    ...
    if line.startswith('DECL') or line.startswith(
            'DEFA') or line.startswith('AUTO'):
        postMass = True
    ...
    if line.startswith('PRES'):
        # find ACE and CT2 PRES residues, set presFlag = False for adding to
        # rtp file
        name = line.split()[1]
...
# initiation of temporary storage for dihedrals and impropers containing
# wildcards
dihWilds = []
impWilds = []
...
for line in parFile:
    ...
    if line.startswith('NONBONDED'):
        ...
        for wilds in impWilds:
            cq = wilds[4]
            # converstion to kJ
            # Need a factor 2 here too of course!!!
            cq = str(float(cq) * 2 * kcal2kJ)
            bonFile.write(
                wilds[0] +
                '\t' +
                wilds[1] +
    ...
    if len(line) > 0:
        ...
        if bonds and not angles:
            ...
            if line[0] != comment and segments[0] != 'BONDS':
                ...
                typej = segments[1]
                Kb = segments[2]
                # converstion from kcal/mole/A**2 -> kJ/mole/nm**2 incl factor
                # 2 (see definitions)
                ...
                if line[0] != comment and segments[0] != 'CMAP':
                    ...
                    try:  # if float cmap parameters start
                        ...
                        for i in range(len(segments)):
                            ...
                            # cmapFile.write(line+'\n')
                    except BaseException:  # if not float it's a string and defines the cmap atom types
                        ...
                        cmapFile.write('\n\n' + segments[0] + ' ' + segments[1] + ' ' + segments[2] + ' ' + segments[3] +
                                       ' ' + segments[7] + ' ' + str(cmapType) + ' ' + segments[-1] + ' ' + segments[-1] + '\\' + '\n')
...
#                        None
        ...
        if nonbonded and not hbond:
            ...
            if line[0] != comment and segments[0] != 'NONBONDED' and segments[0] != 'cutnb' and segments[0] != 'CUTNB':
                ...
                sigma = str(2 * float(RminHalf) / (10.0 * 2.0**(1.0 / 6.0)))
                ...
                if len(segments) > 6:
                    ...
                    if not isinstance(segments[5], str):  # not string?
                        ...
                        # conversion to gromacs units
                        sigma14 = str(2 * float(Rmin14Half) /
                                      (10.0 * 2.0**(1.0 / 6.0)))
                ...
                # add special types of TIP3p model, in both with and without
                # HEAVY_H
                noChargeComment = ''
                if type == 'HT':
                    ...
                    nbFile.write(type +
                                 ...
                                 '\t' +
                                 str(4 *
                                     float(element2mass[type2element[type]])) +
                                 '\t' +
                                 charge +
                                 '\t' +
                                 ptype +
