...
version() {
  ...
  startdate=$(git log --follow --date=local --pretty=format:%ad --diff-filter=A "${rootdir}")
  ncommits=$(git shortlog -s --date=local --since="${startdate}" "${rootdir}" "${this}" | awk '{n += $1}; END{print n}')
